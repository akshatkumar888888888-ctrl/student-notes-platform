...
export default function App() {
  ...
  const [selectedSubject, setSelectedSubject] = useState<string>(
    subjects.find((s) => s.class_id === "12" && s.stream_id === "science")?.id || ""
  );
  ...
  useEffect(() => {
    const firstMatch = subjects.find(
      (s) => s.class_id === selectedClass && s.stream_id === selectedStream
    );
    setSelectedSubject(firstMatch?.id || "");
    setSearchQuery("");
