...
// Real admin secret lives only in the server environment (set ADMIN_SECRET
// in your .env file). Never hardcode it in client source.
const ADMIN_SECRET = process.env.ADMIN_SECRET || "";

// Endpoint to validate an admin code without ever exposing the real secret to the client
...
app.delete("/api/notes/:id", async (req, res) => {
  try {
    ...
    // If a caller sends the admin header, it must match the real server-side
    // secret. Requests with no admin header still fall through as a normal
    // "delete my own upload" request (unchanged), but a WRONG admin code is
    // now rejected instead of silently doing nothing.
